...
def isWordGuessed(secretWord, lettersGuessed):
    ...
    returns: boolean, True if all the letters of secretWord are in lettersGuessed;
      False otherwise
    '''
    return (secretWord.lower() == (''.join(lettersGuessed)).lower())
...
def getGuessedWord(secretWord, lettersGuessed):
    ...
    '''
    return(''.join(map(lambda x: x + ' ' if lettersGuessed.count(x)>0 else '_ ', secretWord)))
...
def getAvailableLetters(lettersGuessed):
    ...
    return(''.join(map(lambda x: x if lettersGuessed.count(x) == 0 else '', alpha)))
...
def hangman(secretWord):
    ...
    * After each round, you should also display to the user the 
    ...
    while(not isWordGuessed(secretWord, lettersGuessed)):
        ...
        guess = guess.lower()
        if (lettersGuessed.count(guess) >0):
