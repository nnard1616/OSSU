...
I = eye(num_labels);
Z2 = [ones(m,1) X]*Theta1'; %5000x25
A2 = sigmoid(Z2);           %5000x25
Z3 = [ones(m,1) A2]*Theta2';%5000x10
A3 = sigmoid(Z3);           %5000x10

H = A3;
%feedforward cost function
for i = 1:m
  J = J + ( log(H)(i,:)*(-1*I(:,y(i))) - (log(1-H))(i,:)*(1 - I(:,y(i))));
endfor;
J = J/m;

%regularized feedforward cost function
J = J + lambda/(2*m)*(sum((Theta1(:,2:size(Theta1)(2)).^2) * ones(size(Theta1)(2)-1,1)) + sum((Theta2(:,2:size(Theta2)(2)).^2) * ones(size(Theta2)(2)-1,1)));
